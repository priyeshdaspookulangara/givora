...
function distributeCommissions($pdo, $new_member_id, $sponsor_id, $package_type) {
    ...
    $matrix_type = ($package_type === 'Leadership_15000') ? 'Matrix_Income_P2' : 'Matrix_Income_P1';
    ...
    for ($level = 0; $level < count($level_percentages); $level++) {
        ...
        // Log transactions
        $level_num = $level + 1;
        $stmt = $pdo->prepare("INSERT INTO transactions (member_id, type, amount, wallet_type, status, description) VALUES (?, ?, ?, 'User_Wallet', 'Credit', ?)");
        $stmt->execute([$curr_parent, $matrix_type, $user_part, "Matrix L{$level_num} Commission (60%) from " . $new_member_id]);

        $stmt = $pdo->prepare("INSERT INTO transactions (member_id, type, amount, wallet_type, status, description) VALUES (?, ?, ?, 'Company_Wallet', 'Credit', ?)");
        $stmt->execute([$curr_parent, $matrix_type, $company_part, "Matrix L{$level_num} Commission (40%) from " . $new_member_id]);
